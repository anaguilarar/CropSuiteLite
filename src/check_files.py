...
from .nc_tools import get_netcdf_extent, get_rows_cols
...
def check_climate_data(options: dict) -> list:
    ...
    if os.path.exists(temp.replace('.tif', '.nc')) and os.path.exists(prec.replace('.tif', '.nc')):
        ...
        temp_dims = get_rows_cols(temp)
        prec_dims = get_rows_cols(prec)
    ...
    if os.path.exists(temp.replace('.tif', '.nc')) and os.path.exists(prec.replace('.tif', '.nc')):
        temp, prec = temp.replace('.tif', '.nc'), prec.replace('.tif', '.nc')
        temp_extent = get_netcdf_extent(temp)
        prec_extent = get_netcdf_extent(prec)
