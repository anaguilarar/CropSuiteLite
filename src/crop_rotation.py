...
from .read_plant_params import read_crop_parameterizations_files
from .read_climate_ini import read_ini_file
from .data_tools import read_tif_file_with_bands, get_geotiff_extent, write_geotiff, read_raster_to_array


def crop_rotation(config_file):
    config_ini = read_ini_file(config_file)
    plant_params = read_crop_parameterizations_files(config_ini['files'].get('plant_param_dir', 'plant_params'))
...
def compute_combinations(crop_a, crop_b, gc_a, gc_b, resting, results_folder):
    ...
    extent = get_geotiff_extent(os.path.join(results_folder, crop_a, 'cr_temp.tif'))
    suitabilities_a = read_tif_file_with_bands(os.path.join(results_folder, crop_a, 'cr_temp.tif')).astype(np.int8)
    suitabilities_b = read_tif_file_with_bands(os.path.join(results_folder, crop_b, 'cr_temp.tif')).astype(np.int8)
    ...
    suit_a[suit_b <= 0] = -1
    suit_b[suit_a <= 0] = -1
    write_geotiff(results, f'1-{crop_a}_climatesuitability.tif', suit_a, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    write_geotiff(results, f'2-{crop_b}_climatesuitability.tif', suit_b, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    sm = suit_a.astype(np.int16) + suit_b.astype(np.int16)
    sm[sm <= 0] = -1
    write_geotiff(results, f'1+2_climatesuitability.tif', sm, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)

    date_a = dates[..., 0]
    date_b = dates[..., 1]
    date_a[suit_a == -1] = -1
    date_b[suit_b == -1] = -1
    write_geotiff(results, f'1-{crop_a}_climate_sowingdate.tif', date_a, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    write_geotiff(results, f'2-{crop_b}_climate_sowingdate.tif', date_b, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    ...
    write_geotiff(results, f'1-{crop_a}_climate_harvestdate.tif', hv_date_a, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    write_geotiff(results, f'2-{crop_b}_climate_harvestdate.tif', hv_date_b, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)    
    del hv_date_a, hv_date_b

    soil_a = read_raster_to_array(os.path.join(results_folder, crop_a, 'soil_suitability.tif')).astype(np.int8)
    suit_a = np.min([soil_a, suit_a], axis=0)
    suit_a[(soil_a == -1) | suitabilities_a[..., 0] == -1] = -1
    soil_b = read_raster_to_array(os.path.join(results_folder, crop_b, 'soil_suitability.tif')).astype(np.int8)
    suit_b = np.min([soil_b, suit_b], axis=0)
    suit_b[(soil_b == -1) | suitabilities_b[..., 0] == -1] = -1
    suit_b[suit_a <= 0] = -1
    suit_a[suit_b <= 0] = -1
    suit_b[suit_a <= 0] = -1
    write_geotiff(results, f'1-{crop_a}_cropsuitability.tif', suit_a, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    write_geotiff(results, f'2-{crop_b}_cropsuitability.tif', suit_b, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    sm = suit_a.astype(np.int16) + suit_b.astype(np.int16)
    sm[sm <= 0] = -1
    write_geotiff(results, f'1+2_cropsuitability.tif', sm, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    ...
    date_b[date_a < 0] = -1
    date_a[date_b < 0] = -1
    write_geotiff(results, f'1-{crop_a}_crop_sowingdate.tif', date_a, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    write_geotiff(results, f'2-{crop_b}_crop_sowingdate.tif', date_b, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    date_a[date_a > 0] = (date_a[date_a > 0] + gc_a) % 365
    date_b[date_b > 0] = (date_b[date_b > 0] + gc_b) % 365
    write_geotiff(results, f'1-{crop_a}_crop_harvestdate.tif', date_a, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
    write_geotiff(results, f'2-{crop_b}_crop_harvestdate.tif', date_b, extent=extent, nodata_value=-1, dtype='int', inhibit_message=True)
