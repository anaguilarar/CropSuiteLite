...
def get_suitability_val_dict(forms, form_type, value):
    """
    ...
    Example structure:  {'form_type_1': {'formula': func_1, 'min_val': min_val_1, 'max_val': max_val_1}, ...}
...
def climsuit_new_xarray(climate_config, ):
    pass
...
def process_day_climsuit_memopt(args):
    ...
    """
    day, growing_cycle, temperature, precipitation, plant_params_forumlas, wintercrop, water_mask, irrigation,\
    ...
    temp[water_mask == 1] = (get_suitability_val_dict(plant_params_forumlas, 'temp', temp[water_mask == 1] / 10) * 100)
    ...
    if irrigation == 0:
        ...
        prec[water_mask == 1] = (get_suitability_val_dict(plant_params_forumlas, 'prec', prec[water_mask == 1] / 10) * 100).astype(np.int8)
    ...
    if crop_failures.ndim > 1:
        ...
        failure_suit[water_mask == 1] = (get_suitability_val_dict(plant_params_forumlas, 'freqcropfail', crop_failures[water_mask == 1]/100) * 100).astype(np.int8)
    else:
...
def process_day_concfut(day, len_growing_cycle, temp_shape, temp_dtype, temp_shm_name,
                              prec_shape, prec_dtype, prec_shm_name, 
                              climate_config, plant, wintercrop, water_mask,
                        crop_failures, vernalization_params, lethal, lethal_params, sowprec_params,
                        photoperiod, photoperiod_params, max_consec_dry_days, dry_day_prec, dursowing, sowingtemp, max_prec_val, 
                        max_prec_dur, additional_conditions
                        ):
        try: 
        ...
        existing_temp_shm = shared_memory.SharedMemory(name=temp_shm_name)
        existing_prec_shm = shared_memory.SharedMemory(name=prec_shm_name)
        
        temperature = np.ndarray(temp_shape, dtype=temp_dtype, buffer=existing_temp_shm.buf)
        precipitation = np.ndarray(prec_shape, dtype=prec_dtype, buffer=existing_prec_shm.buf)
        
        plant_params = rpp.read_crop_parameterizations_files(climate_config['files']['plant_param_dir'], verbose = False)
        plant_params_formulas = rpp.get_plant_param_interp_forms_dict(plant_params, climate_config)
        is_irrigated = int(climate_config['options']['irrigation'])

        if crop_failures.ndim == 2: # type: ignore
            return process_day_climsuit_memopt([day, len_growing_cycle, temperature, precipitation, plant_params_formulas[plant], wintercrop, water_mask,
                                        is_irrigated, crop_failures, vernalization_params, lethal, lethal_params, sowprec_params,
            ...
            return process_day_climsuit_memopt([day, len_growing_cycle, temperature, precipitation, plant_params_formulas[plant], wintercrop, water_mask,
                                        is_irrigated, crop_failures[day], vernalization_params, lethal, lethal_params, sowprec_params,
                                        photoperiod, photoperiod_params, max_consec_dry_days, dry_day_prec, dursowing, sowingtemp, max_prec_val, max_prec_dur, additional_conditions])
...
def climsuit_new(climate_config, extent, temperature, precipitation, land_sea_mask, plant_params, results_path, plant, area_name) -> list:
    ...
    no_threads, av_ram = get_cpu_ram()
    worker_flag = climate_config['options'].get('max_workers', None)
    if worker_flag is not None:
        no_threads = int(worker_flag)

    print(f'Using {no_threads} threads')
    ...
    if worker_flag is None:
        ...
        max_proc = int(worker_flag)
    print(f'Using {max_proc} processes')
    ...
    if (len(rrpcf_files) > 0) and climate_config['climatevariability'].get('consider_variability', False):
        ...
        crop_failures = read_area_from_netcdf_list(rrpcf_files, overlap=False, extent=extent, dayslices=True)
        ...
        if crop_failures.shape[0] == 1:
            ...
            if crop_failures.shape != temperature.shape[:2]:
                crop_failures = interpolate_array(crop_failures, temperature.shape, order=0)
        else:
            if crop_failures.transpose(1, 2, 0).shape != temperature.shape:
                crop_failures = interpolate_array(crop_failures.transpose(1, 2, 0), temperature.shape, order=0).transpose(2, 0, 1)
            ...
            while True:
                ...
                if max_proc == 1:
                    ...
                    with concurrent.futures.ProcessPoolExecutor(max_workers=max_proc) as executor:
                        future_to_day ={executor.submit(process_day_concfut,
                        ...
                        for future in concurrent.futures.as_completed(future_to_day):
                           ...
                           try:
                               rs = future.result()
                                ...
                                days_with_error.append(day)
                                print(f"Request for day {day} generated an exception: {exc}")
                    if len(days_with_error)>0:
                        # repeat only for those days with an error
